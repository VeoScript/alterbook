...
const FootBar: FoorBarProps = ({ user }) => {
  ...
  return (
    <View style={tw`flex-row items-center justify-between w-full px-5 py-3 border-line-top`}>
      <Text style={tw`flex-1 text-regular text-base text-accent-4`}>@{user.username}</Text>
      <TouchableOpacity
        activeOpacity={0.5}
        style={tw`flex-1 flex-row items-center justify-end w-full max-w-[8rem] px-3 py-1 rounded-md bg-accent-2`}
